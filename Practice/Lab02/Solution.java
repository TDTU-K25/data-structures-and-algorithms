public class Solution {
    ...
    public static boolean isBracketMatching(String str) {
        MyStack<Character> stk = new MyStack<Character>();
        char[] sequenceOfChars = str.toCharArray();

        for (Character ch : sequenceOfChars) {
            // left delimiters (opening brackets)
            if (ch.equals('(') || ch.equals('[') || ch.equals('{')) {
                stk.push(ch);
            } else if (ch.equals(')') || ch.equals(']') || ch.equals('}')) { // right delimiters
                                                                             // (closing brackets)
                if (stk.isEmpty()) { // Case: too many closing brackets
                    return false;
                } else {
                    String bracket = Character.toString(stk.pop()) + Character.toString(ch);
                    if (!bracket.equals("()") && !bracket.equals("[]") && !bracket.equals("{}")) {
                        return false;
                    }
                }
            } else { // if character is a letter or number or special character => Skip
                continue;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(isBracketMatching("a[5]"));
        System.out.println(isBracketMatching("a{5]"));
        System.out.println(isBracketMatching("a(5))"));
        System.out.println(isBracketMatching("a[[5]"));
        System.out.println(isBracketMatching("a[(1 + 4)]"));
        System.out.println();
